...
(function($) {
    $.Box = function(params) {
        ...
        {
            $('#Box').css("top", Math.max(0, (($(window).height() - $('#Box').outerHeight()) / 3) + 
        ...
        else if(params.position.zone == "right")
        {
            $('#Box').css("top", Math.max(0, (($(window).height() - $('#Box').outerHeight()) / 3) + 
                                                $(window).scrollTop()) + "px");
            $('#Box').css("left", Math.max(0, (($(window).width() - $('#Box').outerWidth())) + 
                                                            $(window).scrollLeft()) + "px");
        } 
        else
        {
            $('#Box').css("top", (params.position.offsetY)?
                (params.position.offsetY):(Math.max(0, (($(window).height() - $('#Box').outerHeight()) / 3) + 
                $(window).scrollTop()) + "px"));
            // $('#dialogBox').css("bottom", 0);                                                                                  
            $('#Box').css("left", (params.position.offsetX)?(params.position.offsetX):(Math.max(0, (($(window).width() - $('#Box').outerWidth()) / 2) + 
                                                                                                $(window).scrollLeft()) + "px"));
        }
        ...
        if(params.draggable) {
            ...
            $('#Box div.header').css('cursor', 'move').on("mousedown", function(e) {
    ...
    $.Box.content = function(newContent) {
        ...
        if(newContent) {
            $('#Box .content').html(newContent);
        } else {
            return $('#Box .content').html();
        }
    }

    $.Box.title = function(newTitle) {
        // Prevent using function without dialog opened
        if(!$.Dialog.opened) {
            return false;
        }
        ...
        if(newTitle) {
            $('#Box .header span').html(newTitle);
        } else {
            return $('#Box .header span').html();
        }
    }
    $.Box.close = function() {
        // Prevent using function without dialog opened
